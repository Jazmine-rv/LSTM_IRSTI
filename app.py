...
Nota para mí: este es el archivo principal de la aplicación Streamlit.
Lo estructuré en 6 pestañas para que sea más fácil de navegar.
La idea es que el usuario pueda ver el problema, la arquitectura,
entrenar el modelo y hacer predicciones en vivo.
"""
...
# Esto lo hice así porque quería entender cómo funciona una LSTM por dentro,
# no solo usar Keras y que haga todo mágicamente.
from lstm_model_numpy import LSTMModel as NumpyLSTM

# ============================================================
# CONFIGURACIÓN DE PÁGINA Y ESTILO
# ============================================================
# Acá configuro el layout y los colores de la app.
# Los colores los saqué de la paleta de IRSTI, quería que se vea
...
# Paleta IRSTI - definí estos colores para mantener consistencia
# en toda la aplicación. Me costó encontrar el tono de rojo que
# quería, al final usé #E63946 que es el que usan en la facu.
COLOR_BG = "#FFFFFF"
...
# CSS personalizado - acá pasé un buen rato ajustando los estilos.
# Quería que las tarjetas tuvieran bordes sutiles y sombras suaves,
...
# Estos generadores los hice para simular diferentes evoluciones de pacientes.
# Al principio tenía un solo generador, pero después me di cuenta de que
# necesitaba varios perfiles para que el tutorial sea más interesante.
# La idea es que el usuario pueda ver cómo se comporta la LSTM con
...
def generar_paciente_estable(n=300):
    ...
    # Para el paciente estable, uso valores que oscilan suavemente
    # alrededor de los rangos normales. Le puse un poco de ruido
    ...
    # El riesgo es bajo y estable, con pequeñas variaciones.
    # Me aseguro de que nunca supere 0.3 con np.clip.
...
def generar_paciente_deterioro(n=300):
    ...
    # Acá el paciente empieza bien pero va empeorando lentamente.
    # Le puse una tendencia lineal ascendente al riesgo.
    t = np.arange(n)
    ...
    # El riesgo sube de 0.1 a casi 1 al final.
    # La pendiente la calculé para que el deterioro sea gradual.
...
def generar_paciente_critico_repentino(n=300):
    ...
    # Este fue el más difícil de hacer. Quería simular un paciente
    # que está estable y de repente se pone crítico, como en una sepsis.
    # Puse el punto crítico en t=200, que es aproximadamente las 2/3 partes.
    t = np.arange(n)
    punto_critico = 200
    ...
    # El riesgo salta de ~0.2 a ~0.85 en el punto crítico.
    # Esto debería ser un buen desafío para la LSTM.
    riesgo = 0.1 + 0.05 * np.sin(2 * np.pi * t / 30) + np.random.normal(0, 0.02, n)
...
def generar_paciente_mejoria(n=300):
    ...
    # Quería un caso optimista también, donde el paciente responde
    # bien al tratamiento. Es el inverso del deterioro.
    t = np.arange(n)
    punto_mejoria = 150
    ...
    # El riesgo empieza alto y va bajando. Usé una exponencial negativa
    # para que la mejora sea más rápida al principio y luego se estabilice.
...
def generar_dataset_irsti(tipo="deterioro", n=300, incluir_vitales=True):
    ...
    # Esta función es la interfaz principal para generar datos.
    # Si el tipo es "combinado", concateno los 4 perfiles.
    # Me pareció útil para tener un dataset más variado.
    ...
    if tipo == "combinado":
        # Divido en 4 partes iguales y concateno los 4 perfiles.
        # Ojo: cada generador usa n_parte=75, pero la función original
        # espera n=300. Esto hace que cada perfil tenga 75 puntos.
        n_parte = n // 4
        partes = []
        for gen, nombre in [
...
def baseline_persistencia_corregida(y_train, y_test):
    ...
    Esto lo tuve que corregir varias veces porque el baseline
    me daba resultados raros. Al principio predecía el último valor
    de test para todo, pero eso no es persistencia real.
    
    La persistencia real es: predecir que el valor va a ser igual
    al valor inmediatamente anterior. Así que el primer valor de
    test lo predigo con el último de entrenamiento, y el resto
    con el valor anterior de test.
    
    Fuente: Hyndman, R.J., & Athanasopoulos, G. (2018). 
    Forecasting: principles and practice. OTexts.
    """
    # Aplanar si son 2D - esto me pasó cuando usaba el scaler
...
# GLOSARIO LATERAL
# ============================================================
# El glosario lo hice porque me pareció importante que el usuario
# pueda consultar conceptos sin tener que salir de la app.
# Usé la estructura de diccionario porque es fácil de expandir.
# La idea es que cada concepto tenga: definición, fórmula, y
# cómo se comporta en valores bajo/medio/alto.

# La analogía la inventé yo para que sea más fácil de entender.
...
# Barra lateral con el glosario - elegí este lugar porque
# es accesible pero no interrumpe el flujo principal.
with st.sidebar:
    ...
    st.markdown("---")
    # Le puse el link al blog de colah porque es la referencia
    # que más me ayudó a entender las LSTM.
    st.markdown(" ¿Preferís un video? [Mirá esta explicación](https://www.youtube.com/watch?v=f6PaCo-NfJA)")
    
    st.markdown("---")
    st.markdown("### 🧠 Motor de IA")
    st.info("🧠 **NumPy Manual** ")


# ============================================================
# INICIALIZACIÓN DE ESTADO
# ============================================================
# Para que los datos persistan entre interacciones.
# Al principio no lo hacía y los gráficos se perdían cada vez que
# cambiaba algo. Me llevó un rato entender cómo funciona
# st.session_state.
...
# TABS PRINCIPALES
# ============================================================
# Decidí usar tabs para organizar la información en 6 pasos.
# Me pareció más intuitivo que tener todo en una sola página
# con scroll infinito.
...
# ------------------------------------------------------------
with tab1:
    # Acá quiero presentar el problema de forma atractiva.
    # El título grande con el nombre del proyecto es para que
    # quede claro de qué se trata.
...
# ------------------------------------------------------------
with tab2:
    # Esta pestaña la hice para explicar los dos tipos de tareas
    # que puede hacer una red neuronal. 
...
with tab3:
    ...
    # Quería poner un ejemplo concreto para que la gente entienda
    # cómo funciona la LSTM paso a paso. Usé la analogía del
    # enfermero porque me pareció más intuitiva que hablar de
    # matrices y vectores.
    st.markdown("""
    <div style="
    ...
    with col2:
        ...
        # Hice este diagrama con matplotlib porque quería algo
        # que pudiera personalizar completamente. Me costó un
        # poco posicionar los elementos pero creo que quedó claro.
...
# ------------------------------------------------------------
with tab4:
    # Esta es la pestaña más larga y compleja (me costó bastante) . Acá el usuario
    # puede elegir el perfil de paciente, ajustar hiperparámetros
    ...
    with col1:
        ...
        if st.button("🔄 Generar Dataset", use_container_width=True):
            ...
            with col2:
                # Hice un gráfico con plotly para que sea interactivo.
                # El usuario puede hacer zoom y ver los detalles. 
                fig = go.Figure()
        ...
        if st.session_state.data_df is not None:
            ...
            # ── TAMAÑO DE VENTANA ──
            # La ventana es el número de pasos atrás que el modelo
            # usa para predecir. Me costó decidir el rango de valores.
            ...
            # Puse estos mensajes de ayuda para que el usuario
            # sepa qué está haciendo. Me pareció importante.
            if window_size <= 8:
    ...
    if train_button and st.session_state.data_df is not None:
        with st.spinner("🧠 Entrenando LSTM con NumPy"):
            ...
            # Solo uso la columna de riesgo para entrenar.
            # Podría usar todas las variables pero quería mantenerlo simple.
            ...
            # Me llevó un tiempo entender cómo pasar los parámetros
            # correctamente. Al principio me confundía el orden.
            ...
            # Forzar predicciones a [0,1] (por si la LSTM da valores negativos)
            # Esto me pasó varias veces y no sabía por qué. Después me di cuenta
            # de que a veces la LSTM predice valores fuera del rango.
            ...
            # Esto lo hice para que el usuario tenga una referencia
            # de si el modelo está aprendiendo o no.
            ...
            # ============================================================
            # Esta parte la agregué para que el usuario sepa si su
            # modelo está funcionando bien y qué puede mejorar.
            # Me inspiré en los diagnósticos que dan los médicos.
            st.markdown("---")
...
# ------------------------------------------------------------
# TAB 5 — PREDICCIÓN EN VIVO + EXPLICABILIDAD 
# ------------------------------------------------------------
with tab5:
    # Esta pestaña está pensada para que el usuario juegue con
    # los signos vitales y vea cómo cambia el riesgo en tiempo real.
    # La parte de explicabilidad la agregué porque me pareció
    # importante que el usuario entienda por qué el modelo
    ...
    if st.session_state.model is None:
        ...
        # CALCULAR RIESGO 
        # Esta función la hice para que el usuario vea cómo se
        # calcula el riesgo a partir de los signos vitales.
        # Usé una combinación lineal de las desviaciones de
        # los rangos normales.
...
# ------------------------------------------------------------
with tab6:
    # Esta pestaña la agregué porque me pareció importante que
    # el usuario entienda que esto es un proyecto educativo y
    # no un sistema médico real.
